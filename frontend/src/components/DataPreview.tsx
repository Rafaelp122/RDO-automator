...
export function DataPreview({ sheets, onSheetsChange, onComplete }: DataPreviewProps) {
  ...
  const handleColumnToggle = (sheetName: string, columnName: string) => {
    onSheetsChange(sheets.map(s => {
      if (s.name !== sheetName) return s;
      const alreadySelected = s.selectedColumns.includes(columnName);
      return {
        ...s,
        selectedColumns: alreadySelected 
          ? s.selectedColumns.filter(c => c !== columnName)
          : [...s.selectedColumns, columnName]
      };
    }));
  };

  const selectedSheetsCount = sheets.filter(s => s.selected).length;
  const totalSelectedColumns = sheets.reduce((acc, s) => acc + (s.selected ? s.selectedColumns.length : 0), 0);
        ...
        {sheets.map(sheet => (
          ...
          >
            <input 
              ...
              onChange={(e) => handleSheetToggle(sheet.name, e)}
              onClick={(e) => e.stopPropagation()}
            />
            <span className={`text-sm font-medium ${
              activeTabName === sheet.name || sheet.selected ? 'text-[var(--color-primary)]' : 'text-slate-500'
            }`}>
              {sheet.name}
            </span>
          </div>
                  ...
                  <th key={idx} className="p-3">
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <input 
                        ...
                        onChange={() => handleColumnToggle(activeSheet.name, header)}
