...
from src.excel import preview_source, preview_template, ReportGenerator

router = APIRouter(prefix="/api")


@router.post("/preview/source", response_model=SourcePreviewResponse)
async def preview_source_route(file: UploadFile = File(...)):
...
async def preview_template_route(file: UploadFile = File(...)):
...
async def generate_route(
    ...
    output = ReportGenerator(
        source_bytes, template_bytes, source.filename, template.filename, config
    ).generate()
